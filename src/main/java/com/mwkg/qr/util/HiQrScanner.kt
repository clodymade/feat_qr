...
object HiQrScanner : HiScanner {
    ...
    override fun start(activity: Activity, callback: (HiResult) -> Unit) {
        TODO("Not yet implemented") // Placeholder for implementation
    }
    ...
    fun start(activity: Activity, previewView: PreviewView, callback: (HiResult) -> Unit) {
        ...
        if (!activity.hiHasPermissions(reqPermissions)) {
            activity.requestPermissions(reqPermissions, HiGlobal.PermissionReqCodes.CAMERA)
            return
        }
    ...
    private fun handleBarcodeResult(barcode: Barcode) {
        ...
        if (qrResult.isNotEmpty()) {
            callback?.let { it(HiResult.HiQrResult(qrResult, "")) }
        } else {
            callback?.let { it(HiResult.HiQrResult("", "QR code could not be recognized.")) }
        }
    }
}
